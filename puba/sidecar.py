...
"""Bibliographic record read/write with per-field provenance merge."""
...
from .artifacts import bib_record_path, bib_yaml_path, read_bib, write_bib
from .io import now_iso, sha256_file
...
def bib_path(analysis_dir: Path) -> Path:
    ...
    return bib_record_path(analysis_dir)


def load_bib(analysis_dir: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    """Load the bib record. Returns (fields, provenance), or empty dicts if absent."""
    raw = read_bib(analysis_dir)
    if raw is None:
        return {}, {}
    raw = dict(raw)
...
def save_bib(
    ...
    ordered: dict[str, Any] = {"schema_version": 1}
    ...
    # JSON is committed atomically before stale YAML is removed, so an
    # interruption can only leave the documented both-forms-present state.
    write_bib(analysis_dir, ordered, fmt="json")
    legacy_path = bib_yaml_path(analysis_dir)
...
def _load_raw(analysis_dir: Path) -> dict[str, Any]:
    """Load the authoritative bib record including tool-managed keys."""
    return read_bib(analysis_dir) or {}
...
def apply_patch(
    ...
    """Apply a field-level patch to the authoritative bib record, stamping sticky provenance.
    ...
    authoritative record byte-identical on disk and returns fields_changed=[].

    source: must match ^(human|tool:[\\w.-]+)$.
    Returns a result dict with fields_changed, cleared_review, bib_path.
    """
    ...
    for field in patch_fields:
        if field.startswith("_"):
            raise ValueError(
                ...
                "These are tool-managed; use supported bib commands instead."
            )
    ...
    if not fields_changed and not cleared_review:
        return {
            "fields_changed": [],
            "cleared_review": False,
            "bib_path": str(bib_path(analysis_dir)),
        }
...
def load(pdf_path: Path) -> dict[str, Any]:
    """Public API: load the full authoritative bib record as a flat dict."""
    from .state import analysis_dir as get_analysis_dir
    return read_bib(get_analysis_dir(pdf_path)) or {}
...
def load_clean(
    ...
    """Load the authoritative bib record for the show API.
