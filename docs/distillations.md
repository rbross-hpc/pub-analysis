...
After any desired stages have run, use `puba summarize paper.pdf` to write
`paper.summary.md` beside the PDF. It is a deterministic, read-only,
point-in-time sanity-check snapshot of the available bib, status, figures, and
distillations. It is not auto-regenerated by other commands and can go stale;
run `puba summarize` again when you want a fresh snapshot.
...
| `puba summarize <pdf>` | Write a point-in-time sanity-check snapshot beside the PDF; it is not auto-regenerated and can go stale |
